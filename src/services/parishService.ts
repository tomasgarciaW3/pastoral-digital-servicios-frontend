    ...
    setTimeout(() => {
      ...
      const filtered = mockParishes.filter(
        (parish) =>
          ...
          parish.address.toLowerCase().includes(query.toLowerCase())
      );
...
export const getParishDetails = async (parishId: number): Promise<any | null> => {
  ...
  return new Promise((resolve) => {
    ...
    // Mock services with new structure
    const services = [
      {
        id: 1,
        name: "Misa",
 ...
 * @param bounds - Map bounding box (min/max lat/lon), optional countryId filter
